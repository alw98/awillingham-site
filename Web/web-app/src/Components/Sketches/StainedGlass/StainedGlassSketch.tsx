...
export const StainedGlassSketch: React.FC<StainedGlassSketchProps> = observer(({ themeStore, propsStore }) => {
	const sketch = useCallback((s: p5) => {
		let strips: GlassStrip[];

		const getRandomDist = () => {
			// calculate the mean and standard deviation based on the parameters
			let mean = propsStore.minStripDist + (propsStore.maxStripDist - propsStore.minStripDist) * propsStore.averageStripDistPct;
		...
		const init = () => {
			...
			if(propsStore.showGrid){
				for(let x = 0; x <= propsStore.width / 2; x += propsStore.gridCellWidth) {
					for(let y = 0; y <= propsStore.height; y += propsStore.gridCellHeight) {
						s.circle(x, y, 5);
					}
				}
			}
			...
			for(let i = 0; i < propsStore.stripCount; ++i) {
				...
				const width = propsStore.minStripWidth + Math.random() * (propsStore.maxStripWidth - propsStore.minStripWidth);
				const stripDeltaTheta = Math.PI / (Math.floor(Math.random() * 3) + 4);
				strips.push({pos: new p5.Vector(x, y), theta, color, width, stripDeltaTheta});
			}
		}
		...
		s.draw = () => {
			...
			s.scale(-1, 1);
			s.image(s.get(0, 0, propsStore.width / 2, propsStore.height), -propsStore.width, 0, propsStore.width / 2, propsStore.height);
			s.scale(-1, 1);
			s.noStroke();
			...
			for(let i = strips.length - 1; i >= 0; --i) {
				...
				strip.theta += Math.round(Math.random()) ? -strip.stripDeltaTheta : strip.stripDeltaTheta;
				if(strip.pos.x < 0 || strip.pos.x > propsStore.width / 2 || strip.pos.y < 0 || strip.pos.y > propsStore.height) {
					strips.splice(i, 1);
				}
			}
		};
...
export const StainedGlassDefaultPropsStore = observable<StainedGlassSketchPropsStore>({
	name: 'StainedGlass',
	backgroundColor: '',
	width: 320,
	height: 320,
